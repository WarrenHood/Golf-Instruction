...
function hideToolbar(){
	...
	slideInterval = setInterval(function(){
		if(parseFloat(tb.style.left) > -screen.width + parseFloat(g('toggleToolbar').style.width)*2.5)tb.style.left = parseFloat(tb.style.left) - screen.width*tbSpeed +'px';
...
function frontLoad(f){
	current = f;
	g('currentPage').value = current || 'cover';
	front.src = subdir + f + '.html';
}
function backLoad(f){
	back.src = subdir + f + '.html';
}
